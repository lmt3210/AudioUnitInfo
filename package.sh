...
APP_ENTITLEMENTS="AudioUnitInfo.entitlements"
APP=./AudioUnitInfo.xcarchive/Products/Applications/AudioUnitInfo.app
...
if [ "${ACTION}" == "package" ]; then
    ...
    codesign -s "${DEV_ID_APP}" -f --timestamp -o runtime \
        --entitlements "${APP_ENTITLEMENTS}" tmp_app/AudioUnitInfo.app

    # Build component package
    pkgbuild --root tmp_app --identifier com.larrymtaylor.AudioUnitInfo \
    ...
    productbuild --sign "${DEV_ID_INST}" --distribution dist.plist \
        --product requirements.plist AudioUnitInfo.pkg
    ...
    xcrun notarytool submit AudioUnitInfo.pkg --keychain-profile \
        "LT_PASSWORD" --wait

    # Staple notary ticket to product
    xcrun stapler staple AudioUnitInfo.pkg
